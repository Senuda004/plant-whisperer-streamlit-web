...
with left:
    st.markdown('<div class="pw-title">🌿 Plant Whisperer</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="pw-subtitle">Upload a plant image → get prediction, confidence, and Grad-CAM explainability.</div>',
        unsafe_allow_html=True
    )
...
with colA:
    ...
    st.markdown('<div class="pw-card">', unsafe_allow_html=True)
    st.markdown("### Notes")
    st.write("• Grad-CAM is generated by your EC2 model API.")
    st.write("• Scores shown are the raw softmax outputs returned by the API.")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if uploaded is None:
        ...
        if result is None:
            ...
            with s2:
                ...
                if conf is None:
                    st.markdown('<span class="pw-badge">—</span>', unsafe_allow_html=True)
                else:
...
st.caption("Built with Streamlit • Backend inference served from your EC2 Flask API")
